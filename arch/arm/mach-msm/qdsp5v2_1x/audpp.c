			    ...
			    void (*getevent) (void *ptr, size_t len))
{
	...
	case AUDPP_MSG_STATUS_MSG:{
			unsigned cid = msg[0];
			pr_aud_info("audpp: status %d %d %d\n", cid, msg[1],
				msg[2]);
	...
	case AUDPP_MSG_CFG_MSG:
		if (msg[0] == AUDPP_MSG_ENA_ENA) {
			pr_aud_info("audpp: ENABLE\n");
			audpp->enabled = 1;
			audpp_broadcast(audpp, id, msg);
		} else if (msg[0] == AUDPP_MSG_ENA_DIS) {
			pr_aud_info("audpp: DISABLE\n");
			audpp->enabled = 0;
			wake_up(&audpp->event_wait);
			audpp_broadcast(audpp, id, msg);
		} else {
			pr_aud_err("audpp: invalid config msg %d\n", msg[0]);
		}
		break;
	...
	case ADSP_MESSAGE_ID:
		pr_aud_info("audpp: enable/disable done\n");
		break;
	default:
		pr_aud_info("audpp: unhandled msg id %x\n", id);
	}
}
		     ...
		     unsigned *queueid)
{
	...
	if (audpp->codec_cnt[codec_type] < audpp->decoder_count) {
		...
		MM_AUD_INFO("different codec running\n");
	} else {
		...
		audpp->codec_max_instances = max_instance;
		MM_AUD_INFO("same codec running\n");
	}
	...
	if (lidx < audpp->dec_database->num_dec) {
		...
		*queueid =
		...
		MM_AUD_INFO("%s: decid = 0x%08x, module_name = %s, queueid = %d\n",
			__func__, decid, *module_name, *queueid);
	}
done:
	MM_AUD_INFO("%s: return decid %d\n", __func__, decid);
				 ...
				 const char *buf, size_t count)
{
	...
	if (!rc &&
		...
		pr_aud_err("%s: Not a valid Concurrency case\n", __func__);
		rc = -EINVAL;
	}
done:
	mutex_unlock(audpp->lock_dec);
