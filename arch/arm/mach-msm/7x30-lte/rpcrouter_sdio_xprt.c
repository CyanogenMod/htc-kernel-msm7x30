...
#if 1 /* HTC : modify the debug mech */
#define SDIO_XPRT_DBG(x...) do { \
    if (sdio_dbg_flag & DBG_SDIO_RPC_DBG || msm_sdio_xprt_debug_mask) \
        printk(KERN_DEBUG x); \
    } while(0)
#define SDIO_XPRT_INFO(x...) do { \
    if (sdio_dbg_flag & DBG_SDIO_RPC_INFO || msm_sdio_xprt_debug_mask) \
        printk(KERN_INFO x); \
    } while (0)
...
static void dbg_dump_buf(u8 *prestr, const u8 *buf, int len)
{
    ...
    unsigned char aggre_buf[BUF_SIZE];
    int i, count=0, limit=BUF_SIZE;
	int data_size=0, quit=0, offset=0, ret=0;

    if((sdio_dbg_flag & DBG_SDIO_RPC_RAW_DATA) == false)
        return ;

	/* buf checking */
	if(BUF_SIZE / 3 < DATA_PER_LINE + 1)
		return ;

	printk("[%15.15s][%4.4d] rpc data start: \n",prestr, len);

	while(!quit) {

		if(count + DATA_PER_LINE >= len) {
			data_size = len - count;
			quit = true;
		}
		else
			data_size = DATA_PER_LINE;
		...
		/* format string */
		offset = 0, limit = BUF_SIZE;
	    for (i = 0; i < data_size ; i++) {
			ret = snprintf(aggre_buf+offset, limit, "%02x ", buf[i+count]);
			if (limit > 3) {
			    offset += ret;
			    limit -= ret;
			}
...
#else /* HTC_ELSE : original dbg mach (not used in htc) */
enum {
					...
					enum write_data_type type)
{
	...
	case HEADER:
		...
		sdio_write_pkt = kmalloc(sizeof(struct sdio_write_data_struct), GFP_ATOMIC);
		sdio_write_pkt->write_len = len +
					    ((struct rr_header *)data)->size;
		/* Andy: Used GFP_ATOMIC flag under irq_disabled situation */
		sdio_write_pkt->write_data = kmalloc(sdio_write_pkt->write_len, GFP_ATOMIC);
...
static void sdio_xprt_write_data(struct work_struct *work)
{
	...
	while (!list_empty(&write_list)) {
        ...
        /* HTC */
        dbg_dump_buf("SDIO_RPC->WD#", sdio_write_data->write_data, sdio_write_data->write_len);
        /* HTC_END */
...
{
	printk(KERN_INFO "sdio_xprt Called %s\n", __func__);
...
find_buf:
	...
	while ((read_avail = sdio_read_avail(sdio_remote_xprt.channel->handle))
		...
		/* HTC */
        dbg_dump_buf("SDIO_RPC->RD#", data, avail);
        /* HTC_END */
