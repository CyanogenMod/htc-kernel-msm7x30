...
extern void *get_pmem_virt_addr(void **phys_addr, const char *name, int length, int *size);
...
char *get_fb_addr(void)
{
	...
	printk(KERN_ERR "%s: impossible to be here\n", __func__);
	return 0;
}
#endif
			...
			if (mapped<3) {
				for (i=0; i<3; i++) {
					if (phys_addr[i] == (void *)usb_pjt_info.latest_offset)
						break;
					if (!phys_addr[i]) {
						void *pmem_vbase = NULL;
						void *pmem_base = NULL;
						int pmem_size = 0;
						const char pmem_name[] = "pmem_adsp2";
						phys_addr[i] = (void *)usb_pjt_info.latest_offset;
						pmem_vbase = get_pmem_virt_addr(&pmem_base, pmem_name, strlen(pmem_name), &pmem_size);
						virt_addr[i] = pmem_vbase;
						mapped++;
						break;
					}
				}
